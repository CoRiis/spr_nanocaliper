...
class SPRFitter:
    ...
    def SquareLoss(self, x):
        ...
        if not self.print_every is None and self._optim_counter % self.print_every == 0:
            SPRFitter._print_current(self._optim_counter, B,
                                     self.mdls[0].get_dyn_rates_as_dict(rates), chi2)
    ...
    def minimize(self, max_signal, guess_rates_dict, method='Nelder-Mead', **kwargs):
        ...
        if self.print_every is not None:
            SPRFitter._print_current(self._optim_counter, best_B,
                                     best_rates_dict, best_chi2)

        # B,rates=self._get_B_and_all_rates_from_input(res.get('x'))
        # ,'res_B': B,'res_rates': rates}
        return {'B': best_B, 'rates': best_rates, 'rates_dict': best_rates_dict, 'chi2': best_chi2, 'iterations': self._optim_counter, 'res': res}
