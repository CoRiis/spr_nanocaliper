...
class ODE:
    ...
    def _update(self):
        ...
        if self._use == 'jnp':
            embed_dyn = jnp.zeros((len(self.R.molecule_i), len(
                self.R.molecule_i) - len(self.concentration_functions)))
            for k, i in self.dyn_molecule_i.items():
                embed_dyn = embed_dyn.at[self.R.molecule_i[k], i].set(1.0)
        else:
            embed_dyn = np.zeros((len(self.R.molecule_i), len(
                self.R.molecule_i) - len(self.concentration_functions)))
            for k, i in self.dyn_molecule_i.items():
        ...
        if self._use == 'jnp':
            embed_func = jnp.zeros(
                (len(self.R.molecule_i), len(self.concentration_functions)))
